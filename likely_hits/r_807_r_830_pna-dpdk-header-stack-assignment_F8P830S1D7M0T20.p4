 typedef bit<32> PortIdUint_t;
  ...
  control MainControlImpl(inout parsed_headers_t hdrs, inout user_meta_t umeta, in pna_main_input_metadata_t istd, inout pna_main_output_metadata_t ostd)() {
   ...
   table header_mod {     key = {       hdrs.mac.da : exact;     }     actions = {       encap_one_tunnel_layer_ipv4;       decap_one_tunnel_layer_just_before_eth;       NoAction;     }     const default_action = NoAction;   }
   apply {     if (hdrs.mac.isValid()) {   header_mod.apply(); }     send_to_port(((PortId_t) (1)));   }
 }
